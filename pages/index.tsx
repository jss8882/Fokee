...
const Home: NextPage = () => {
	...
	const visibleBtn = "text-[#F3B562] px-2 my-5 font-semibold";

	const invisibleBtn = "opacity-30 text-[#5C4B51] px-2 my-5 font-semibold";
	...
	return (
		<div className="w-full h-full bg-[#F5EFDF]">
			<div className="flex items-center justify-end bg-[#FDFDFA] ">
				...
				<Link href="/setting">
					<a className="px-2 py-2 text-[#ECC392] text-[20px] font-light">
						<AiFillSetting />
					</a>
				</Link>
			</div>
			<div className="text-5xl flex justify-center pb-10 text-[#5C4B51] bg-[#FDFDFA]  font-black">FOKEE(logo사진)</div>
			<div className="bg-[#F5EFDF] flex justify-center">
				<button onClick={() => setIsCapsuleMode(false)} className={!isCapsuleMode ? visibleBtn : invisibleBtn}>
					지금 있는 캡슐은
				</button>
				<button onClick={() => setIsCapsuleMode(true)} className={isCapsuleMode ? visibleBtn : invisibleBtn}>
					위시리스트
				</button>
			</div>
			{!isCapsuleMode ? (
				<div>
					{capsulesInlab.map((capsule, index) => {
						...
						return <BriefCapsuleInfo key={index} capsuleColor={capsule.color} capsuleName={capsule.capsuleName} />;
					})}
				</div>
			) : (
				<div className="flex flex-col items-center px-10 pb-20 bg-[#F5EFDF] opacity-33 h-[420px]">
					<div className="py-5 flex w-[280px] justify-between">
						<label className=" text-[#5C4B51] font-semibold">캡슐 이름</label>
						<input className="bg-white" />
					</div>
					<div className="py-5 flex w-[280px] justify-between">
						<label className="text-[#5C4B51] font-semibold">브랜드</label>
						<input className="bg-white" />
					</div>
					<div className="pt-5 pb-20 flex w-[280px] justify-between">
						<label className="text-[#5C4B51] font-semibold">URL</label>
						<input className="bg-white" />
					</div>
					<button className="bg-[#F3B562] rounded-full w-[150px] text-[20px] py-3 font-bold text-[#5C4B51]">SUBMIT</button>
				</div>
			)}
			<div style={{ visibility: !capsuleClick ? "hidden" : "visible" }}>
				<div>뿅 나는 캡슐 정보</div>
